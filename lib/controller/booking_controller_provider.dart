...
import 'package:theinstallers/model/product_type_model.dart';
...
class BookingControllerProvider extends ChangeNotifier {
  ...
  void updateSelectedDateTime(DateTime date) {
   ...
   Future<void> newBooking({
    ...
    String? dateAndTimeOfBooking, required String tasktypeid, String? typeOfMaterial,
    String? paymentModeId, String? remarks, String? notes, String? desription,String? nearestServiceCenterId, 
    String? landmark,String? quantity, required String salutation_id,String? productTypeId,
    required List<int> subProductIds,
    List<File>? formFileVideos,
    ...
    try {
      ...
      MapEntry('type_of_product_id', productTypeId ?? ''),
      
       
    ];
    
     // Add each sub_product_id to formData
      for (var id in subProductIds) {
        formData.fields.add(MapEntry('sub_product_id[]', id.toString()));
      }
    ...
    if (formFileVideos != null && formFileVideos.isNotEmpty) {
      for (int i = 0; i < formFileVideos.length; i++) {
        final File formFileVideo = formFileVideos[i];
        final bytes = await formFileVideo.readAsBytes();
        final fileName = formFileVideo.path.split('/').last;

        formData.files.add(
          MapEntry(
            'service_video[$i][video_file]',
            MultipartFile.fromBytes(bytes, filename: fileName),
          ),
        );
      ...
      else if (errorResponse != null && errorResponse['type_of_product_id'] != null) {
        throw _responseMessage = errorResponse['type_of_product_id'][0];
      }
      else if (errorResponse != null && errorResponse['sub_product_id'] != null) {
        throw _responseMessage = errorResponse['sub_product_id'][0];
      }
...
*/
  ...
  int? get selectedProductTypeId => _selectedProductTypeId;
  int? _selectedProductTypeId;  
  void setSelectedProductTypeId(int? id) {
    _selectedProductTypeId = id;
     print("select Product Type Id: $selectedProductTypeId");
    notifyListeners();
  }

  

  Future<ProductTypeModel?> fetchProductType() async {
    try {
      final token = await DatabaseControllerProvider().getToken();
      const urlProductType = AppUrl.produtTypeUri;
      final response = await dio.get(
        urlProductType,
        options: Options(
          headers: {
      ...
      return ProductTypeModel.fromJson(response.data);
    } catch (error) {
      if (kDebugMode) {
  ...
  int? get selectedSubProductTypeId => _selectedSubProductTypeId;
  int? _selectedSubProductTypeId;  
  void setSelectedSubProductTypeId(int? id) {
    _selectedSubProductTypeId = id;
     print("select Sub Product Type Id: $selectedSubProductTypeId");
    notifyListeners();
  }

  

  Future<SubProductTypeModel?> fetchSubProductType(String typeOfProductId) async {
    try {
      final token = await DatabaseControllerProvider().getToken();
      final urlSubProductType = '${AppUrl.subProdutTypeUri}?type_of_product_id=${typeOfProductId.toString()}';
      final response = await dio.get(
        urlSubProductType,
        options: Options(
          headers: {
      ...
      return SubProductTypeModel.fromJson(response.data);
      // print(response.data.toString());
    } catch (error) {
      if (kDebugMode) {
        print('Error fetching sub product type: $error');
      }
      return null;
  ...
  Future<PaymentModeModel?> fetchPaymentMode() async {
   ...
   Future<void> updateBooking({required int id,String? address, required String fullName, String? email,String? phone, 
    ...
    String? paymentModeId, String? remarks, String? notes, String? landmarks,String? nearestServiceCenterId, 
     String? quantities, required String salutation_id, String? productTypeId,
    ...
    final body = {
      ...
      'coordinate': nearestServiceCenterId,
      'salut': salutation_id,
      'type_of_product_id': productTypeId,
        if (subProductIds != null) 'sub_product_id': subProductIds,
    };
    // print(body);
  ...
  Future<void> cancelBooking({required int id,BuildContext? context,
   
    
  }) async {
    final urlCancelService = '${AppUrl.cancelServiceUri}?service_id=$id';
    // print(urlUpdateService);
    final token = await DatabaseControllerProvider().getToken();
    _isLoading = true;
    notifyListeners();
    ...
    try {
      final response = await dio.post(urlCancelService,
      ...
      if (response.statusCode == 200) {
        _isLoading = false;
        _responseMessage = "Booking canceled successfully! ";
        notifyListeners();
