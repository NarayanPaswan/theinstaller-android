...
class DrawEditorFormView extends StatefulWidget {
  final String selectedDrawImagePath;

  const DrawEditorFormView({Key? key, required this.selectedDrawImagePath})
      : super(key: key);

  @override
  State<DrawEditorFormView> createState() => _DrawEditorFormViewState();
}

class _DrawEditorFormViewState extends State<DrawEditorFormView> {
  String _imagePath = '';
  ...
  void initState() {
    ...
    _imagePath = '';
    if (widget.selectedDrawImagePath.isNotEmpty) {
      _imagePath = widget.selectedDrawImagePath;
    }
    print("image path is ${widget.selectedDrawImagePath}");
  }
          ...
          appBar: AppBar(
            ...
            actions: [
              IconButton(
                ...
                Uint8List? editedDrawImage = await screenshotController.capture();
                if(!mounted) return;
                Navigator.of(context).pop(editedDrawImage);
